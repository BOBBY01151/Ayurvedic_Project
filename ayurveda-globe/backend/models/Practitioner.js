...
const practitionerSchema = new mongoose.Schema({
  ...
  title: {
    ...
    default: 'Ayurvedic Practitioner'
  },
  specialties: [{
    type: String,
    enum: [
  ...
  clinic: {
    ...
    ref: 'Clinic',
    required: [true, 'Practitioner must be associated with a clinic']
  },
  bio: {
    type: String,
    trim: true,
  ...
  languages: [{
    type: String,
    enum: ['English', 'Sinhala', 'Tamil', 'Hindi', 'Sanskrit'],
  ...
  user: {
    ...
    ref: 'User',
    required: function() {
      // Only required if practitioner has login access
...
practitionerSchema.index({ specialties: 1 });
practitionerSchema.index({ clinic: 1 });
practitionerSchema.index({ 'rating.average': -1 });
practitionerSchema.index({ isActive: 1, isVerified: 1 });

// Virtual for average rating display
practitionerSchema.virtual('displayRating').get(function() {
  return this.rating.count > 0 ? this.rating.average.toFixed(1) : 'No ratings yet';
});

// Method to check availability for a specific day
practitionerSchema.methods.isAvailableOn = function(dayName) {
  const day = dayName.toLowerCase();
  return this.availability[day] && this.availability[day].available;
};

// Method to get consultation fee in different currencies
practitionerSchema.methods.getConsultationFeeIn = function(targetCurrency) {
  // TODO: Implement currency conversion
  if (this.consultationFee.currency === targetCurrency) {
    return this.consultationFee.amount;
  }
